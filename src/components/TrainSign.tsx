...
interface TrainSignProps {
  /** Kept for API compatibility; colors come from design tokens */
  theme?: Theme;
...
const TrainSign: React.FC<TrainSignProps> = ({ events, isAdmin, onEditClick }) => {
  ...
  const containerClass =
    'hidden md:flex flex-col items-end justify-center leading-tight px-3 py-1.5 font-bold uppercase overflow-hidden min-h-[2.5rem] bg-[var(--fs-train-bg)] border border-[color:var(--fs-train-border)]';
    ...
    >
      {phase === 'main' ? (
        <div key="main" className="flex flex-col items-end animate-fade-in text-left">
          <span className="text-[7px] tracking-[0.3em] text-[color:var(--fs-train-subtext)]">
            GREENBUSH LINE · QUINCY CTR
          </span>
          <span className="text-[11px] font-bold tracking-[0.12em] leading-tight mt-0.5 text-[color:var(--fs-train-primary)]">
            WEYMOUTH LANDING / E. BRAINTREE
          </span>
          <span className="text-[7px] tracking-[0.3em] mt-0.5 text-[color:var(--fs-train-accent)]">ZONE 2 · $7.50</span>
        </div>
      ) : currentEvent ? (
        <div
          key={scrollKey}
          className="flex flex-col items-end animate-slide-up text-[10px] tracking-[0.15em] whitespace-nowrap text-[color:var(--fs-train-event)]"
        >
